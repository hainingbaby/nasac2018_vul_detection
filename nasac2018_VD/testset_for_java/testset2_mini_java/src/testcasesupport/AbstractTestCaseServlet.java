...
public abstract class AbstractTestCaseServlet extends AbstractTestCaseServletBase 
{
    ...
    public abstract void foo_bar(HttpServletRequest request,
            ...
            throws IOException 
    {
        ...
        try 
        {
            foo_bar(request, response);
            
            response.getWriter().println("<br>Completed foo_bar() without Throwable in Servlet " + servletName);
        } 
        catch (Throwable throwableException) 
        {
            response.getWriter().println("<br>Caught thowable from foo_bar() in Servlet " + servletName);
