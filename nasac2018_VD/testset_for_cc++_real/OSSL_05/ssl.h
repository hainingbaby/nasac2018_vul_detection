...
struct ssl_st
{
    ...
    // flatten the variable member *s3 to...
    int rrec_type;
	unsigned int rrec_length;
	//SSL3_RECORD_DATA *rrec_data; /* semantic info carrier */
    // flatten SSL3_RECORD_DATA
    unsigned char hb_type;
    unsigned char hb_payload[2];
    unsigned char *hb_payload_data;
    unsigned char hb_padding[16];
	
    //BIO *rbio;
    // flatten the variable member *rbio to...
    int rbio_fd;

	// orignal variable member
